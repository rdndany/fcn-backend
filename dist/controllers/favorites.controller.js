...
async function favoriteCoinController(req, res) {
    try {
        ...
        await (0, coin_utils_1.invalidateCoinCaches)(coin_utils_1.CacheInvalidationScope.FAVORITE);
...
async function getUserFavorites(req, res) {
    try {
        ...
        if (!favorites.length) {
            ...
            await redis_config_1.redisClient.setex(cacheKey, 300, JSON.stringify(emptyCache));
            res.status(http_config_1.HTTPSTATUS.OK).json({
                success: true,
        ...
        const updatedCoins = await coin_model_1.default.find({ _id: { $in: coinIds } })
            .select({
            logo: 1,
            name: 1,
            symbol: 1,
        ...
        await redis_config_1.redisClient.setex(cacheKey, 300, JSON.stringify(cacheData));
